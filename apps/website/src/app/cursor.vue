<template>
	<div v-show="isEnabled" :class="$style['cursor-container-raw']">
		<div ref="ringRef" :class="$style['cursor-ring-raw']"></div>
		<div ref="dotRef" :class="$style['cursor-dot-raw']"></div>
	</div>
</template>
...
const onOver = (e: PointerEvent) => {
	if ((e.target as HTMLElement | null)?.closest(INTERACT_SELECTOR)) {
		gsap.to(ringRef.value, {
			...
			overwrite: 'auto',
			ease: 'back.out(1.7)',
			borderColor: 'oklch(0.6 0.25 230 / 0.6)',
			backgroundColor: 'oklch(0.6 0.25 230 / 0.1)',
		});
...
const onOut = (e: PointerEvent) => {
	if ((e.target as HTMLElement | null)?.closest(INTERACT_SELECTOR)) {
		gsap.to(ringRef.value, {
			...
			borderColor: 'oklch(0.7 0.22 230 / 0.3)',
		});
...
const onDown = (e: PointerEvent) => {
	...
	ripple.className = $style['cursor-ripple-raw'];
...
.cursor-container-raw {
	...
	.cursor-dot-raw {
		...
		width: 8px;
		height: 8px;
		background-color: @fantasy8;
		box-shadow:
			0 0 10px @fantasy8,
			0 0 20px color-mix(in oklch, @fantasy8, transparent 50%);
	}
	...
	.cursor-ring-raw {
		...
		border: 1.5px solid color-mix(in oklch, @fantasy8, transparent 70%);
		box-shadow: inset 0 0 8px color-mix(in oklch, @fantasy8, transparent 90%);
		transition:
...
.cursor-ripple-raw {
	...
	border: 1px solid color-mix(in oklch, @fantasy8, transparent 50%);
	box-shadow: 0 0 15px color-mix(in oklch, @fantasy8, transparent 70%);
